<template>
  <div>
    <div class="form-button">
      <button class="btn btn-primary" @click="showReplyToCommentForm = !showReplyToCommentForm">
        <span v-if="!showReplyToCommentForm">{{ $t('user.blog_system_pages.general_settings.comment_section.open_reply_comment_form') }}</span>
        <span v-else @click="clearReplyCommentForm">{{ $t('user.blog_system_pages.general_settings.comment_section.close_reply_comment_form') }}</span>
      </button>
    </div>
    <div v-show="showReplyToCommentForm">
...
export default {
  ...
  methods: {
    clearReplyCommentForm () {
      if (this.form.full_name === '' || this.form.email === '' || this.form.message === '' || this.form.comment_reply_is_public === true || this.form.privacy_policy_comment_reply === true) {
        this.form.full_name = ''
        this.form.email = ''
    ...
    async replyToComment () {
      ...
      console.log('> blog article comment id: ', this.blogArticleCommentId)
      await this.form
        .post(fullApiUrl, {
          blogArticleCommentId: this.form.blog_article_comment_id,

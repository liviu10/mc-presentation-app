        ...
        <div v-if="articleComment.blog_article_comment_replies">
          <div v-for="articleCommentReply in articleComment.blog_article_comment_replies" :key="articleCommentReply.id" class="card article-comments-list-reply-card">
            <div class="card-body">
              ...
              <div v-if="articleComment.blog_article_comment_replies.blog_article_comment_reply_replies">
                <div v-for="articleReplyToCommentReply in articleComment.blog_article_comment_replies.blog_article_comment_reply_replies" :key="articleReplyToCommentReply.id" class="card article-comments-list-reply-card">
                  <div class="card-body">
                    <h5 class="card-title">
                      <span>{{ articleReplyToCommentReply.full_name }}</span> i-a răspuns lui
                      <span>{{ articleComment.full_name }}</span> pe
                      <span>{{ new Date(articleReplyToCommentReply.created_at).toLocaleDateString('ro-RO', { day: 'numeric', month: 'long', year: 'numeric' }) }}</span>
                    </h5>
                    <p class="card-text">
                      {{ articleReplyToCommentReply.comment_reply }}
                    </p>
                    <div class="form-button">
                      <button class="btn btn-primary" @click="showReplyToCommentReplyForm = !showReplyToCommentReplyForm">
                        <span v-if="!showReplyToCommentReplyForm">Răspunde</span>
                        <span v-else>Anulează</span>
                      </button>
                    </div>

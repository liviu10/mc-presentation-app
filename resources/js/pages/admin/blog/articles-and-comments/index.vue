...
export default {
  ...
  methods: {
    ...
    async editBlogCategoryAndSubcategory (row) {
      this.selectedDataToEdit = row
      return this.selectedDataToEdit
    },
    async deleteCategoryAndSubcategory (row) {
      if (!this.user) {
        Swal.fire({
    ...
    async deleteAllBlogCategoriesAndSubcategories () {
      if (!this.user) {
        Swal.fire({
          title: 'Unauthenticated User Title',
          text: 'Unauthenticated User Text Message',
  ...
  metaInfo () {
    return { title: 'Admin - Blog articles and comments' }
  }
}
</script>
